...
class TestDMRPPFileGeneration:
    def test_generate_dmrpp_success(self, mocker, process_factory, granule_id) -> None:
        ...
        # Mocks
        mocker.patch(
            "dmrpp_generator.main.DMRPPGenerator.upload_file_to_s3",
            return_value={granule_id: f"s3://{granule_name}.dmrpp"},
        )
        mocker.patch(
            "cumulus_process.Process.fetch_all",
            return_value={
                "input_key": [os.path.join(process_instance.path, granule_name)]
            },
        )
        mocker.patch("os.remove", return_value=granule_name)
        mocker.patch(
            "cumulus_process.s3.download",
            return_value=f"{process_instance.path}/{granule_name}",
        )
    ...
    def test_fail_on_no_matching_regex(
        ...
        mocker.patch(
            "cumulus_process.s3.download",
            return_value=f"{process_instance.path}/{hdf_name}",
        )
