def test_hdf4(fixture_path, dmrpp_cli):
    ...
    assert (
        dmrpp_cli.get_dmrpp_command(dmrpp_meta={}, file_full_path=file_full_path)
...
def test_option(fixture_path, dmrpp_cli):
    ...
    """
    file_full_path = f"{fixture_path}/test_file.matlab.hdf5"
    test_meta = {"options": [{"flag": "-c"}]}
    assert (
        dmrpp_cli.get_dmrpp_command(dmrpp_meta=test_meta, file_full_path=file_full_path)
