...
class DMRPPGenerator(Process):
    ...
    def __init__(self, **kwargs):
        ...
        self.enable_subprocess_logging = self.dmrpp_meta.get(
            ...
            os.getenv("ENABLE_SUBPROCESS_LOGGING", "false").lower() == "true",
        )
        ...
        self.logger_to_cw.info(
            f"enable_subprocess_logging: {self.enable_subprocess_logging}"
        )
    ...
    def get_bucket(filename, files, buckets):
        ...
        if not bucket_type:
            raise Exception(
                f"File '{filename}' does not match any file regex defined within the collection definition."
            )
    ...
    def process_cumulus(self):
        ...
        for granule in granules:
            ...
            for file_ in granule["files"]:
                ...
                if not search(f"{self.processing_regex}$", file_["fileName"]):
                    self.logger_to_cw.info(
                        f"{self.dmrpp_version}: regex {self.processing_regex}"
                ...
                output_file_paths = self.dmrpp_generate(
                    input_file=input_file_path, dmrpp_meta=self.dmrpp_meta
                )
                    ...
                    if output_file_path:
                        ...
                        dmrpp_file = {
                            "bucket": self.get_bucket(
                                output_file_basename, collection_files, buckets
                            )["name"],
                            "fileName": output_file_basename,
                            "key": os.path.join(
                                os.path.dirname(file_.get("key")), output_file_basename
                            ),
                            "size": os.path.getsize(output_file_path),
                            "type": self.get_file_type(
                                output_file_basename, collection_files
                            ),
                        }
                        dmrpp_files.append(dmrpp_file)
                        upload_location = (
                            f"s3://{dmrpp_file['bucket']}/{dmrpp_file['key']}"
                        )
                ...
                if len(dmrpp_files) == 0:
                    self.logger_to_cw.warning(
                        f"No dmrpp files were produced for {granule}"
                    )
        ...
        if self.verify_output and not output_generated:
            raise Exception(
                "No dmrpp files were produced and verify_output was enabled."
            )
    ...
    def get_dmrpp_command(self, dmrpp_meta, file_full_path):
        ...
        dmrpp_cmd = (
            f"gen_dmrpp_side_car -i {file_full_path} -U {required_options} {options}"
        )
    ...
    def dmrpp_generate(self, input_file, local=False, dmrpp_meta=None, args=None):
        ...
        file_full_path = (
            input_file
            if local
            else s3.download(input_file, path=self.path, extra=extra)
        )
